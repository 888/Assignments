...
"-e <tlb_eviction_policy> Set TLB eviction policy\n"\
...
void sim_setoptions(int argc, char **argv) {
  ...
  progname = argv[0];
  while ((c = getopt(argc, argv, "p:m:t:j:e:h")) != -1) {
    switch (c) {
    case 'p':
    ...
    case 'j':
      ...
      break;
    case 'e':
      if (optarg == NULL || atoi(optarg) < 0 || !(!strcmp("LRU", optarg) || !strcmp("FIFO", optarg))) {
        ERROR("invalid TLB eviction policy -- must be LRU or FIFO");
        exit(EXIT_FAILURE);
      }
      tlb_eviction_policy = optarg;
      break;
    case 'h':
    default:
      USAGE();
  ...
  printf("tlb_eviction_policy = %s mode\n", tlb_eviction_policy);
