...
%arbitrary
threshold = .75;
%threshold is .75 for notre dame
match = [];
confidence = [];
...
for i=1:size(features1, 1)
    ...
    for j=1:size(features2, 1)
        vect = features1(i, 1:size(features1,2)) - features2(j, 1:size(features2,2));
        dist = sqrt(sum(vect.^2));
        elems = [elems, dist];
    end
    [B,I] = sort(elems,'ascend');
    if B(1)/B(2) < threshold
        pair = [i, I(1)];
        match = [match;pair];
        confidence = [confidence; 1 - B(1)/B(2)];
    end
end
...
[confidences, ind] = sort(confidence, 'descend');
matches = match(ind,:);