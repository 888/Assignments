...
confidences = [];
a=1;
while a<=size(features1, 1)
    elems = [];
    b=1;
    while b<=size(features2, 1)
        vect = features1(a, 1:size(features1,2)) - features2(b, 1:size(features2,2));
        elems = [elems, sqrt(sum(vect.^2))];
        b=b+1;
    end
    [B,I] = sort(elems,'ascend');
    if B(1)/B(2) < 0.75
        match = [match;[a, I(1)]];
        confidences = [confidences; 1 - B(1)/B(2)];
    end
    a=a+1;
end
