    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for np, pp in newPosDist.items():
                allPossible[np] += self.beliefs[p] * pp
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                newPossible[p] = emissionModel[trueDistance] * self.getBeliefDistribution()[p]
            if newPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                output = []
                for num in range(self.numParticles):
                    output.append(util.sample(newPossible))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = ghosts[0:self.numParticles]
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        "*** YOUR CODE HERE ***"
        for index, nd in enumerate(noisyDistances):
            if nd == None:
                for num, part in enumerate(self.particles):
                    self.particles[num] = self.getParticleWithGhostInJail(part, index)
        multi = util.Counter()
        for par in self.particles:
            running = 1;
            for nGhost in range(self.numGhosts):
                if noisyDistances[nGhost] is not None:
                    trueDist = util.manhattanDistance(par[nGhost], pacmanPosition)
                    running *= emissionModels[nGhost][trueDist]
            multi[par] += running
        if multi.totalCount() == 0:
            self.initializeParticles()
            for index, nd in enumerate(noisyDistances):
                if nd == None:
                    for num, part in enumerate(self.particles):
            ...
            for numb in range(self.numParticles):
                end.append(util.sample(multi))
            self.particles = end
