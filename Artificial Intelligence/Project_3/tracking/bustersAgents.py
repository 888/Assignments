...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for p in livingGhostPositionDistributions:
            maxes.append(p.argMax())

        index = 0
        mmax = 0
        wax = maxes[0]
        for key in maxes:
            if mmax < livingGhostPositionDistributions[index][key]:
                mmax = livingGhostPositionDistributions[index][key]
                wax = key
            index += 1
        ...
        for act in legal:
            ...
            takes.append((self.distancer.getDistance(successorPosition, wax), act))
        return min(takes)[1]
