...
def consistent(assignment, csp, var, value):
	"""Question 1"""
	"""YOUR CODE HERE"""
	for rule in csp.binaryConstraints:
		if rule.affects(var):
			other = rule.otherVariable(var)
			if assignment.isAssigned(other):
				if not rule.isSatisfied(assignment.assignedValues[other], value):
					return False
	return True
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for var in domains:
		if not assignment.isAssigned(var):
			tup = (var, len(domains[var]))
			tupList.append(tup)
	low = min(tupList, key = lambda t : t[1])
	for tup in tupList:
		if tup[1] == low[1]:
			degList.append(tup)
	if len(degList) == 1:
		...
		degree = {}
		for varTup in degList:
			degree[varTup[0]] = 0
			for node in csp.binaryConstraints:
				if node.affects(varTup[0]) and not assignment.isAssigned(node.otherVariable(varTup[0])):
					degree[varTup[0]] += 1
		nextVar = max(degree, key = degree.get)
	return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	leasts = []
	for val in values:
		temp = (val, counter(assignment, csp, var, val))
		leasts.append(temp)
	leasts.sort(key = lambda t : t[1])
	values = []
	for tup in leasts:
		values.append(tup[0])
	return values

def counter(assignment, csp, var, value):
	count = 0
	for rule in csp.binaryConstraints:
		if rule.affects(var):
			var2 = rule.otherVariable(var)
			var2vals = assignment.varDomains[var2]
			for val in var2vals:
				if not rule.isSatisfied(val, value):
					count += 1
	return count
...
def forwardChecking(assignment, csp, var, value):
	...
	for rule in csp.binaryConstraints:
		if rule.affects(var):
			var2 = rule.otherVariable(var)
			var2vals = domains[var2]
			if value in var2vals:
				inferences.add((var2, value))
	for ivar, ival in inferences:
		domains[ivar].remove(ival)
		if len(domains[ivar]) == 0:
			for iivar, iival in inferences:
				domains[iivar].add(iival)
			return None
	return inferences
		...
		if consistent(assignment, csp, var, val):
			...
			if inferences is not None:
				...
				if result is not None:
					return result
				for ivar, ival in inferences:
					assignment.varDomains[ivar].add(ival)
		assignment.assignedValues[var] = None
	return None
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	if not consistent(assignment, csp, var, value):
		return None
	if value not in assignment.varDomains[var]:
		return None
	domains = assignment.varDomains
	qq = deque()
	for rule in csp.binaryConstraints:
		...
		if rule.affects(var):
			var2 = rule.otherVariable(var)
			if not assignment.isAssigned(var2):
				qq.append((var, var2, rule))
	while qq:
		pop = qq.popleft()
		revisions = revise(assignment, csp, pop[0], pop[1], pop[2])
		if revisions == None:
			for ivar, ival in inferences:
				domains[ivar].add(ival)
			return None
		elif len(revisions) >= 1:
			for rev in revisions:
				inferences.add(rev)
			for rule in csp.binaryConstraints:
				if rule.affects(pop[1]):
					nvar = rule.otherVariable(pop[1])
					if not assignment.isAssigned(nvar) and nvar != pop[0]:
						qq.append((pop[1], nvar, rule))
	return inferences
