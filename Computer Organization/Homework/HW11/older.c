...
void check_malloc_3() {
    ...
    for (size_t i = 0; i < 43; i++) {
		...
		metadata_t* debug = (metadata_t*) data - 1;
		printf("%s", "after malloc");
		printList();
        ...
        for (size_t j = 0; j < size; j++) {
			...
			metadata_t *temp;
    		printf("------LIST-----\n");
    		for (temp = freelist; temp != NULL; temp = temp->next) { //segfault
        		fprintf(stderr, "(%d,%d)->", temp->block_size, temp->request_size);
