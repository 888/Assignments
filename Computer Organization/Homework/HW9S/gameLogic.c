...
#define ARCHERYMOVE 5


/*
	Move the archery 5 columns to the right when the right button is pressed
	Only move the archery its column + its width is less than 240
*/
...
/*
	...
	Only move the archery its column + the ARCHERYMOVE is greater than 0
*/
...
/*
	Check if the shooting point hit any object
	If the shooting point hit a onject, erase that object
	and update it row and col; update the old object;
	return the an int 
	The shooting point hit the target if it lies in anywhere in
	the bottom border line of the object
	* @param row row where the shooting point is at
	* @param col column where the shooting point is at
	* @param *b pointer to the objects in the game
	* @param *ob pointer to the odl objects in the game
	* @param n integer is the number od objects we are not going to check
			 n depends ont he tme time t
