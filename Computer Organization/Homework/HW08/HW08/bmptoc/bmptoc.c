...
int main(int argc, char *argv[]) {
	...
	for (int start = (int) height - 1; start >= 0; start--) {
		for (unsigned int now = 0; now < width; now++) { //assumes current is the "(0,0)" of the 2d array
			unsigned char blue = ((data_arr[0x36 + (now * 4) + (4 * width * start)]) >> 3) & 0x1F;
			unsigned char green = ((data_arr[0x36 + (now * 4) + (4 * width * start) + 1]) >> 3) & 0x1F;
			unsigned char red = ((data_arr[0x36 + (now * 4) + (4 * width * start) + 2]) >> 3) & 0x1F;
			short pixel = (blue << 10) | (green << 5) | (red);
...
unsigned int getWidth(char *data_arr) {
	unsigned int* width;
	width = (unsigned int*) (data_arr + 0x12); //is this casting ok?
	return *width;
}
...
unsigned int getHeight(char *data_arr) {
	unsigned int* height;
	height = (unsigned int*) (data_arr + 0x16); //is this casting ok?
	return *height;
}
