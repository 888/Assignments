...
void test_pop_back_0(void);
void test_pop_back_1(void);
void test_pop_back_3(void);
...
void ta_destroy(LIST *llist, list_op free_func);
...
void test_push_front_3(void) {
	...
	NODE *t = d->tail;
	assert(t->data == andrew);

	NODE *m = t->prev;
	assert(m != NULL);
	assert(m->data == austin);
...
void test_pop_back_3(void) {
	...
	assert(d->size == 3);
	assert(d->head->data == trenton);
	assert(d->tail->data == austin);
	assert(d->tail->prev != NULL);

	assert(d->tail->prev->data == sai);
	assert(d->tail->prev->prev->next->next == d->tail);
...
void test_to_array_3(void) {
	printf("Test 15: Test To Array 3 - RUN\n");
	LIST *d = create_list();
	person *andrew = create_person("Andrew", 26);
	person *austin = create_person("Austin", 19);
	person *sai = create_person("Sai", 20);
	...
	push_front(d, trenton);
	void **ret = to_array(d);
	void *arr[] = {trenton, sai, austin, andrew};
	for(int x = 0; x < 4; x++) {
		assert(arr[x] == ret[x]);
	}
	free(ret);
	ta_destroy(d, free_person);
	printf("Test 15: Test To Array 3 - SUCCESS\n");
}
